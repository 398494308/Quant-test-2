...
def build_research_windows(config: WindowConfig) -> list[ResearchWindow]:
    ...
    if config.holdout_days < 7:
        raise ValueError(f"holdout_days too small: {config.holdout_days}")

    holdout_start = end_dt - timedelta(days=config.holdout_days - 1)
    eval_last_end = holdout_start - timedelta(days=1)
    if eval_last_end <= start_dt:
        raise ValueError("not enough room for eval windows before holdout")
