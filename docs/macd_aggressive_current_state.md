...
- prompt 里的“最近未压缩轮次表”现在会正确读取当前评分口径的最新历史，不再被全局 compact 索引误切。
- 旧评分口径不会混进当前主表；它们只会以“旧评分口径弱参考”的短摘要形式保留，供模型做低优先级方向启发。
...
- compact 历史现在会写入 `score_regime`，prompt 主摘要只读取当前评分口径；旧口径只保留极短的弱参考摘要，防止长期历史污染但保留少量跨口径启发。
